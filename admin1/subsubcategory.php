...
$My_Class->title("Категория: ".urldecode($cat['name']).' - '.urldecode($subcat['name']));
...
if($_POST['subsubcat'] != "" and $_POST['amount'] != "") {
$subsubcat=urlencode(trim($_POST['subsubcat']));
$amount=$_POST['amount'];

$cat_m = DB::$the->query("SELECT mesto FROM `sel_subsubcategory` WHERE `id_subcat` = {$subcategory} order by `mesto` DESC limit 1 ");
$cat_m = $cat_m->fetch(PDO::FETCH_ASSOC);
$mesto = $cat_m['mesto']+1;

$params = array('name' => $subsubcat, 'amount' => $amount, 'id_subcat' => $subcategory, 'time' => time(), 'mesto' => $mesto);
$q= DB::$the->prepare("INSERT INTO `sel_subsubcategory` (name, amount, id_subcat, time, mesto) VALUES (:name,:amount, :id_subcat, :time, :mesto)");
$q->execute($params);
...
?>
<form action="?cmd=create&category=<?=$category?>&subcategory=<?=$subcategory?>" method="POST">
<div class="form-group col-sm-8">
<div class="input-group input-group-lg">
<span class="input-group-addon"><span class="glyphicon glyphicon-pencil"></span> </span>
<input type="text" placeholder="Название раздела" class="form-control" name="subsubcat">
</div><br />
<div class="input-group input-group-lg">
...
<ol class="breadcrumb">
  ...
  <li><a href="subcategory.php?category=<?=$category;?>"><?=urldecode($cat['name']);?></a></li>
  <li><a href="subsubcategory.php?category=<?=$category;?>&subcategory=<?=$subcategory?>"><?=urldecode($subcat['name']);?></a></li>
  <li><?=urldecode($subsubcat['name']);?></li>
  <li class="active">Редактирование раздела</li>
</ol>
<?	
$row = DB::$the->query("SELECT * FROM `sel_subsubcategory` WHERE `id` = {$subsubcategory} and `id_subcat` = {$subcategory}");
$subsubcat = $row->fetch(PDO::FETCH_ASSOC);
...
DB::$the->prepare("UPDATE sel_subsubcategory SET name=? WHERE id=? ")->execute(array("$name", $subsubcategory)); 
DB::$the->prepare("UPDATE sel_subsubcategory SET amount=? WHERE id=? ")->execute(array("$amount", $subsubcategory)); 
DB::$the->prepare("UPDATE sel_subsubcategory SET mesto=? WHERE id=? ")->execute(array("$mesto", $subsubcategory)); 
...
?>
<form action="?cmd=edit&category=<?=$category?>&subcategory=<?=$subcategory?>&subsubcategory=<?=$subsubcategory?>" method="POST">
<div class="form-group col-sm-8">
<div class="input-group input-group-lg">
<span class="input-group-addon"><span class="glyphicon glyphicon-pencil"></span> </span>
<input type="text" placeholder="<?=urldecode($subsubcat['name'])?>" class="form-control" name="name" value="<?=urldecode($subsubcat['name'])?>">
</div><br />
<div class="input-group input-group-lg">
<span class="input-group-addon"><span class="glyphicon glyphicon-usd"></span> </span>
<input type="text" placeholder="<?=$subsubcat['amount']?>" class="form-control" name="amount" value="<?=$subsubcat['amount']?>">
</div><br />
<div class="input-group input-group-lg">
<span class="input-group-addon"><span class="glyphicon glyphicon-flag"></span> </span>
<input type="text" placeholder="<?=$subsubcat['mesto']?>" class="form-control" name="mesto" value="<?=$subsubcat['mesto']?>">
</div><br />
...
case 'delete':	
$row = DB::$the->query("SELECT name FROM `sel_subsubcategory` WHERE `id` = {$subsubcategory} and `id_subcat` = {$subcategory}");
$subsubcat = $row->fetch(PDO::FETCH_ASSOC);
?>
<ol class="breadcrumb">
  <li><a href="/admin1">Админ-панель</a></li>
  <li><a href="subcategory.php?category=<?=$category;?>"><?=urldecode($cat['name']);?></a></li>
    <li><a href="subsubcategory.php?category=<?=$category;?>&subcategory=<?=$subcategory?>"><?=urldecode($subcat['name']);?></a></li>
  <li class="active">Удаление раздела: <b><?=urldecode($subsubcat['name']);?></b></li>
</ol>
...
<div class="btn-group">
  ...
  <ul class="dropdown-menu" role="menu">
    <li><a href="?cmd=delete&category=<?=$category;?>&subcategory=<?=$subcategory;?>&subsubcategory=<?=$subsubcategory;?>&ok">Да, удалить</a></li>
    <li class="divider"></li>
...
if(isset($_GET['ok'])) {
DB::$the->query("DELETE FROM `sel_subsubcategory` WHERE `id` = {$subsubcategory} ");
DB::$the->query("DELETE FROM `sel_keys` WHERE `id_subsubcat` = {$subsubcategory} ");
...
<a href="key.php?category='.$category.'&subcategory='.$subcategory.'&subsubcategory='.$cat['id'].'"><b>'.urldecode($cat['name']).'</b></a> ('.count($total).')';
echo '<a href="?cmd=edit&category='.$category.'&subcategory='.$subcategory.'&subsubcategory='.$cat['id'].'"> <span class="badge pull-right"><span class="glyphicon glyphicon-pencil"></span> </a>';
echo '<a href="?cmd=delete&category='.$category.'&subcategory='.$subcategory.'&subsubcategory='.$cat['id'].'"> <span class="badge pull-right"><span class="glyphicon glyphicon-remove"></span> </a>';
echo '</span>';
}
echo '</div>';
