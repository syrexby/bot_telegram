...
$total = DB::$the->query("SELECT id_subcat FROM `sel_subsubcategory` WHERE `id_subcat` = {$cat[id]} ");
$total = $total->fetchAll();
	
echo '<span class="list-group-item"><font color="green">['.$cat['mesto'].'] </font>
<a href="subsubcategory.php?category='.$category.'&subcategory='.$cat['id'].'"><b>'.urldecode($cat['name']).'</b></a> ('.count($total).')';
